/** @file SOURCE/parallelSourceNormal.hpp
 *  @brief Parent class to add a normally pulse to the FDTD grids
 *
 *  A class used to generate normally incident current source on all processes
 *
...
public:
     ...
     */
    parallelSourceNormalBase(std::shared_ptr<mpiInterface> gridComm, std::vector<std::shared_ptr<Pulse>> pulse, std::shared_ptr<parallelGrid<T>> grid, double dt, std::array<int,3> loc, std::array<int,3> sz) :
...
public:
     ...
     * @param[in]  sz         size of the soft source
     */
    parallelSourceNormalReal(std::shared_ptr<mpiInterface> gridComm,  std::vector<std::shared_ptr<Pulse>> pulse, real_pgrid_ptr grid, double dt, std::array<int,3> loc, std::array<int,3> sz);
...
public:
     ...
     * @param[in]  sz         size of the soft source
     */
    parallelSourceNormalCplx(std::shared_ptr<mpiInterface> gridComm,  std::vector<std::shared_ptr<Pulse>> pulse, cplx_pgrid_ptr grid, double dt, std::array<int,3> loc, std::array<int,3> sz);
