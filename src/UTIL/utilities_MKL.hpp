...
extern "C"
{
    ...
    void zgeru_ (const int* m, const int* n, const cplx *alpha, const cplx *x, const int* incx, const cplx *y, const int* incy, cplx *a, const int* lda);
    ...
    int sasum_(const int*, const int*, const int*);
...
namespace
{
    ...
    /**
     * @brief      wrapper function to dgetri_
     *
     * @param[in]  n      The order of the matrix A; n≥ 0.
     * @param      a      a(lda,*) contains the factorization of the matrix A, as returned by ?getrf: A = P*L*U. The second dimension of a must be at least max(1,n).
     * @param[in]  lda    The leading dimension of a; lda≥ max(1, n).
     * @param      ipiv   Array, size at least max(1, n). The ipiv array, as returned by ?getrf.
     * @param      work   A workspace array of dimension at least max(1,lwork).
     * @param[in]  lwork  The size of the work array; lwork≥n.
     * @param[in]  info   If info = 0, the execution is successful. If info = -i, the i-th parameter had an illegal value. If info = i, the i-th diagonal element of the factor U is zero, U is singular, and the inversion could not be completed.
     */
    ...
    /**
     * @brief      wrapper to  dgetrf_
     *
     * @param[in]  m     The number of rows in the matrix A (m≥ 0).
     * @param[in]  n     The number of columns in A; n≥ 0.
     * @param      a     Array, size lda by *. Contains the matrix A. The second dimension of a must be at least max(1, n). Overwritten by L and U. The unit diagonal elements of L are not stored.
     * @param[in]  lda   The leading dimension of array a.
     * @param      ipiv  Array, size at least max(1,min(m, n)). The pivot indices; for 1 ≤i≤ min(m, n), row i was interchanged with row ipiv(i).
     * @param      info   If info=0, the execution is successful. If info = -i, the i-th parameter had an illegal value. If info = i, uii is 0. The factorization has been completed, but U is exactly singular. Division by 0 will occur if you use the factor U for solving a system of linear equations.
     */
    void dgetrf_( const int m, const int n, double* a, const int lda, int* ipiv, int* info )
   ...
   /**
     ...
     * @return     DOUBLE. Contains the sum of magnitudes of real and imaginary parts of all elements of the vector..
     */
    double sasum_(const int n, const int* x, const int inc)
        { return ::sasum_(&n,x,&inc);}
