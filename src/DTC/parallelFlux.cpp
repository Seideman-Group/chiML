...
parallelFluxDTCReal::parallelFluxDTCReal(std::shared_ptr<mpiInterface> gridComm, std::string name, double weight, std::array<pgrid_ptr,3> E, std::array<pgrid_ptr,3> H, std::array<int,3> loc, std::array<int,3> sz, bool cross_sec, bool save, bool load, int timeInt, std::vector<double> freqList, DIRECTION propDir, std::array<double,3> d, double dt, double theta, double phi, double psi, double alpha, std::string incd_file, bool SI, double I0, double a) :
    parallelFluxDTC<double>(gridComm, name, weight, E, H, loc, sz, cross_sec, save, load, timeInt, freqList, propDir, d, dt, theta, phi, psi, alpha, incd_file, SI, I0, a)
{
    ...
    if( !(E[2] && H[2] ) )
    {
        if(sz_[0] > 1 && sz_[1] > 1)
        {
            ...
            fInParam_.push_back(makeParamIn(E, H, DIRECTION::Y, false) );
            fInParam_.push_back(makeParamIn(E, H, DIRECTION::Y, true ) );

            fInParam_.push_back(makeParamIn(E, H, DIRECTION::X, false) );
            fInParam_.push_back(makeParamIn(E, H, DIRECTION::X, true ) );
        }
        else if(sz_[0] == 1 && ( (H[2] && H[2]->local_x() != 3) || (E[2] && E[2]->local_x() !=3) ) )
        {
    ...
    else
    {
        if(sz_[0] > 1 && sz_[1] > 1 && sz_[2] > 1)
        {
            ...
            fInParam_.push_back(makeParamIn(E, H, DIRECTION::Z, true) );
            fInParam_.push_back(makeParamIn(E, H, DIRECTION::Z, false) );
        }
        else if(sz_[0] == 1)
        {
            // Single surface assume positive weight
...
parallelFluxDTCCplx::parallelFluxDTCCplx(std::shared_ptr<mpiInterface> gridComm, std::string name, double weight, std::array<pgrid_ptr,3> E, std::array<pgrid_ptr,3> H, std::array<int,3> loc, std::array<int,3> sz, bool cross_sec, bool save, bool load, int timeInt, std::vector<double> freqList, DIRECTION propDir, std::array<double,3> d, double dt, double theta, double phi, double psi, double alpha, std::string incd_file, bool SI, double I0, double a) :
    parallelFluxDTC<cplx>(gridComm, name, weight, E, H, loc, sz, cross_sec, save, load, timeInt, freqList, propDir, d, dt, theta, phi, psi, alpha, incd_file, SI, I0, a)
{
...
parallelFluxDTCCplx::FieldInputParamsFlux parallelFluxDTCCplx::makeParamIn(std::array<pgrid_ptr,3> E, std::array<pgrid_ptr,3> H, DIRECTION dir, bool pl)
{
    ...
    if(E[2])
        toMaster.addIndex_ = {{ E[2]->procLoc(transCor1) - loc_[transCor1], E[2]->procLoc(cor) - loc_[cor] }};
    else
        toMaster.addIndex_ = {{ H[2]->procLoc(transCor1) - loc_[transCor1], H[2]->procLoc(cor) - loc_[cor] }};
...
parallelFluxDTCReal::FieldInputParamsFlux parallelFluxDTCReal::makeParamIn(std::array<pgrid_ptr,3> E, std::array<pgrid_ptr,3> H, DIRECTION dir, bool pl)
{
    FieldInputParamsFlux to_return;
    masterImportDat toMaster;
    ...
    toMaster.szProcOffsetEj_ = constructSzProcOffsetLists( to_return.Ej_dtc_, toMaster.addIndex_, Ej, Ek, corJ, cor, transCor1 );
    toMaster.szProcOffsetEk_ = constructSzProcOffsetLists( to_return.Ek_dtc_, toMaster.addIndex_, Ek, Ej, corK, cor, transCor1 );
    toMaster.szProcOffsetHj_ = constructSzProcOffsetLists( to_return.Hj_dtc_, toMaster.addIndex_, Hj, Hk, corK, cor, transCor1 );
    toMaster.szProcOffsetHk_ = constructSzProcOffsetLists( to_return.Hk_dtc_, toMaster.addIndex_, Hk, Hj, corJ, cor, transCor1 );
