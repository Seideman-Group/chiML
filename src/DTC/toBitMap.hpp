...
template <typename T> void GridToBitMap (std::shared_ptr<Grid<T>> o, std::string filename, std::function<double(T val)> transIn, std::function<bool(T,T)> funcComp)
{
    ...
    std::tie(min, max) = findMinMax(o, funcComp,  transIn );

    if (min == max )
    {
        max != 0.0 ? max *= 1.1 : max =  0.01;
        max != 0.0 ? min *= 0.9 : min =  0.00;
    }
    double diff = max - min;
    ...
    for(int k = 0; k < slMax; ++k)
    {
        for(int i = 0; i < w; ++i)
        {
            ...
            {
                x = i; y = (h-1) - j;
                val = ( transIn( o->point(i,j, k) ) - min)/diff;
...
template <typename T> void GridToBitMap (std::shared_ptr<Grid<T>> o, std::string filename, std::function<double(T val)> transIn, std::function<bool(T,T)> funcComp, std::array<int,3> loc, std::array<int,3> sz)
{
    // std::cout << 2 << std::endl;
    int w = sz[0];
    int h = sz[1];
    ...
    for(int k = 0; k < slMax; ++k)
    {
        for(int i = 0; i < w; ++i)
        {
            ...
            {
                x = i; y = (h-1) - j;
                val = ( transIn(o->point(i+loc[0],j+loc[1], k+loc[2]) ) - min)/diff;
                img[(x+y*w)*3+2] = (unsigned char)(toRValue(val));
                img[(x+y*w)*3+1] = (unsigned char)(toGValue(val));
                img[(x+y*w)*3+0] = (unsigned char)(toBValue(val));
            }
...
template <typename T> void GridToBitMap (std::shared_ptr<Grid<T>> o, std::string filename, std::array<int,3> corInds, std::array<int,3> add_k, std::function<double(T)> transIn, std::function<bool(T,T)> funcComp)
{
    // std::cout << 3 << std::endl;
    int w     = o->n_vec( corInds[0] );
    int h     = o->n_vec( corInds[1] );
    int slMax = o->n_vec( corInds[2] );
    ...
    int x = 0, y = 0;
    double val = 0.0;
    // std::cout << "create dirs" << std::endl;
    boost::filesystem::create_directories( filename.substr(0,filename.size()-3) );
    ...
    for(int k = 0; k < slMax; ++k)
    {
        for(int i = 0; i < w; ++i)
        {
            for(int j = 0; j < h; ++j)
            {
                ...
                val = ( transIn( o->point( i*add_k[1]+j*add_k[2]+k*add_k[0], i*add_k[2]+j*add_k[0]+k*add_k[1], i*add_k[0]+j*add_k[1]+k*add_k[2] ) ) - min)/diff;
                // std::cout << "rgb  " << val << std::endl;
                // std::cout << "R " << std::endl;
                img[(x+y*w)*3+2] = (unsigned char)(toRValue(val));
                // std::cout << "G " << std::endl;
                img[(x+y*w)*3+1] = (unsigned char)(toGValue(val));
        ...
        std::ofstream outMap(filename + std::to_string(k) + ".bmp", std::ios::out | std::ios::binary);
        outMap.write(reinterpret_cast<char *>(bmpfileheader),sizeof(bmpfileheader));
        outMap.write(reinterpret_cast<char *>(bmpinfoheader),sizeof(bmpinfoheader));
        for(int i=0; i<h; ++i)
        {
        ...
        outMap.close();
        // std::cout << "out file maker" << std::endl;
    }
    // std::cout << "out" << std::endl;
}

template <typename T> void GridToBitMap (std::vector<std::shared_ptr<Grid<T>>> o, std::string filename, std::function<double(T val)> transIn, std::function<bool(T,T)> funcComp)
{
    // std::cout << 4 << std::endl;
    int w = o[0]->n_vec(0);
    int h = o[0]->n_vec(1);
    int slMax = o[0]->n_vec(2);
    ...
    for(auto & grid: o)
    {
        std::tie(temp1, temp2) = findMinMax(grid, funcComp,  transIn );
        min += temp1;
        max += temp2;
    }
    ...
    for(int k = 0; k < slMax; ++k)
    {
        for(int i = 0; i < w; ++i)
        {
            for(int j = 0; j < h; ++j)
            {
                ...
                x = i; y = (h-1) - j;
                for(auto& grid : o)
                    val += ( transIn( grid->point(i,j, k) ) - min)/diff;
                img[(x+y*w)*3+2] = (unsigned char)(toRValue(val));
                img[(x+y*w)*3+1] = (unsigned char)(toGValue(val));
                img[(x+y*w)*3+0] = (unsigned char)(toBValue(val));
            }
...
template <typename T> void GridToBitMap (std::vector<std::shared_ptr<Grid<T>>> o, std::string filename, std::function<double(T val)> transIn, std::function<bool(T,T)> funcComp, std::array<int,3> loc, std::array<int,3> sz)
{
    // std::cout << 5 << std::endl;
    int w = sz[0];
    int h = sz[1];
    ...
    for(int k = 0; k < slMax; ++k)
    {
        for(int i = 0; i < w; ++i)
        {
            for(int j = 0; j < h; ++j)
            {
                ...
                x = i; y = (h-1) - j;
                for(auto& grid : o)
                    val += ( transIn( grid->point(i+loc[0],j+loc[1], k+loc[2]) ) - min)/diff;
                img[(x+y*w)*3+2] = (unsigned char)(toRValue(val));
                img[(x+y*w)*3+1] = (unsigned char)(toGValue(val));
                img[(x+y*w)*3+0] = (unsigned char)(toBValue(val));
            }
...
template <typename T> void GridToBitMap (std::vector<std::shared_ptr<Grid<T>>> o, std::string filename, std::array<int,3> corInds, std::array<int,3> add_k, std::function<double(T)> transIn, std::function<bool(T,T)> funcComp)
{
    // std::cout << 6 << std::endl;
    int w     = o[0]->n_vec( corInds[0] );
    int h     = o[0]->n_vec( corInds[1] );
    int slMax = o[0]->n_vec( corInds[2] );
    ...
    for(int k = 0; k < slMax; ++k)
    {
        for(int i = 0; i < w; ++i)
        {
            for(int j = 0; j < h; ++j)
            {
                ...
                x = i; y = (h-1) - j;
                for(auto& grid : o)
                    val += ( transIn( grid->point( i*add_k[1]+j*add_k[2]+k*add_k[0], i*add_k[2]+j*add_k[0]+k*add_k[1], i*add_k[0]+j*add_k[1]+k*add_k[2] ) ) - min)/diff;
                img[(x+y*w)*3+2] = (unsigned char)(toRValue(val));
                img[(x+y*w)*3+1] = (unsigned char)(toGValue(val));
                img[(x+y*w)*3+0] = (unsigned char)(toBValue(val));
            }
...
// Find the min and max only along the diagonal of the matrix
template <typename T> std::pair<double, double> findMinMax(std::shared_ptr<Grid<T>> &o, std::function<bool(T, T)> funcComp, std::function<double(T)> transFunc)
{
    auto minMaxInd = std::minmax_element(o->data(), o->data()+o->size(), funcComp);
    return std::make_pair( transFunc(*minMaxInd.first), transFunc(*minMaxInd.second) );
}
