...
protected:
    ...
    bool outputAvg_; //!< True if only outputting time integrated outputs;
    double t_start_; //!< time to start collecting fields
    double t_end_; //!< time to stop collecting fields
...
public:
     ...
     */
    parallelDetectorBMPReal(std::vector< std::pair< real_pgrid_ptr, std::array<int,3> > >  grid, bool SI, std::array<int,3> loc, std::array<int,3> sz, bool outputAvg, double t_start, double t_end, std::string out_name, GRIDOUTFXN fxn, GRIDOUTTYPE txtType, DTCTYPE type, double timeInterval, double a, double I0, double dt);
    /**
...
public:
     ...
     * @param[in]  dt            time step
     */
    parallelDetectorBMPCplx(std::vector< std::pair< cplx_pgrid_ptr, std::array<int,3> > >  grid, bool SI, std::array<int,3> loc, std::array<int,3> sz, bool outputAvg, double t_start, double t_end, std::string out_name, GRIDOUTFXN fxn, GRIDOUTTYPE txtType, DTCTYPE type, double timeInterval, double a, double I0, double dt);
    /**
     * @brief Outputs to a bmp and potentially text file
     *
     * @param[in] current simulation time
